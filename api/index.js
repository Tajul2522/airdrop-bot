...
const finalizeTasks = async (ctx, user) => {
    ...
    const finalUser = await User.findOneAndUpdate(
        ...
        { $inc: { balance: totalEarned, taskBalance: totalEarned }, pendingTaskReward: 0, actionState: 'IDLE', allTasksFinished: true },
        { new: true }
    );
    const refLink = `https://t.me/${BOT_USERNAME}?start=${user.telegramId}`;
    const summaryMsg = `🎉 <b>Congratulations, ${ctx.from.first_name}!</b>\n\n✅ <b>Tasks:</b> Finished!\n💰 <b>Reward:</b> ${totalEarned} NXRA\n💵 <b>Total:</b> ${finalUser.balance} NXRA\n\n👥 <b>Referrals:</b> ${finalUser.referralCount || 0}\n🔗 <b>Link:</b>\n${refLink}`;
...
// --- ৫. বাটন লিসেনার (bot.hears সবার আগে থাকবে) ---
...
// --- ৭. টেক্সট মেসেজ লিসেনার (সবার শেষে থাকবে) ---
bot.on('text', async (ctx) => {
    ...
    else if (st === 'ASK_LI' && text.includes('linkedin')) await saveTemp('linkedin', 'step_facebook');
