...
// --- 3. Start Command (With Exactly 6 Buttons) ---
bot.start(async (ctx) => {
    try {
        await connectDB();
        ...
        // বাটন লেআউট (ঠিক আপনার চাহিদা অনুযায়ী ৬টি বাটন)
        const keyboard = Markup.inlineKeyboard([
            [Markup.button.webApp('⛏️ Start Daily Mining', APP_URL)], // ১ম সারি: মাইনিং
            [
                Markup.button.callback('📝 Social Tasks', 'tasks'),    // ২য় সারি বাম: টাস্ক
                Markup.button.callback('🎁 Daily Bonus', 'bonus')      // ২য় সারি ডান: বোনাস
            ],
            [
                Markup.button.callback('🏦 Withdraw', 'withdraw_menu'), // ৩য় সারি বাম: উইথড্র
                Markup.button.callback('👥 Referral', 'get_ref')       // ৩য় সারি ডান: রেফারেল
            ],
            [Markup.button.callback('☎️ Support', 'support')]           // ৪র্থ সারি: সাপোর্ট
        ]);

        ctx.replyWithHTML(`👋 <b>Welcome to Nxracoin Reward Bot!</b>\n\n🚀 Mine daily, complete tasks, and invite friends to earn big!`, keyboard);
    } catch (e) { console.error(e); }
});

// --- 4. Task Completion Summary (Dynamic Reward) ---
...
bot.on('text', async (ctx) => {
    ...
    if (steps[user.actionState]) {
        const nextAction = steps[user.actionState];
        const updatedUser = await User.findOneAndUpdate({ telegramId: userId }, { $inc: { balance: TASK_REWARD, taskBalance: TASK_REWARD }, actionState: 'IDLE' }, { new: true });
        ...
        const finalUser = await User.findOneAndUpdate({ telegramId: userId }, { $inc: { balance: TASK_REWARD, taskBalance: TASK_REWARD }, actionState: 'IDLE' }, { new: true });
...
// Extra buttons
bot.action('ask_wallet', async (ctx) => { await User.findOneAndUpdate({ telegramId: ctx.from.id }, { actionState: 'AWAITING_WALLET' }); ctx.reply("Send Wallet:"); });
